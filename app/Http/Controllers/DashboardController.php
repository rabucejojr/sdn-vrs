...
class DashboardController extends Controller
{
    public function __invoke(Request $request): Response
    {
        ...
        return Inertia::render('Dashboard', [
            'stats' => [
                'pending' => TripTicket::when(! $user->isAdmin(), fn ($q) => $q->where('requested_by', $user->id))
            ...
            'calendar' => [
                'year' => $year,
                'month' => $month,
                'bookedDates' => $this->bookedDatesForMonth($monthStart, $monthEnd, $user->isAdmin()),
            ],
    ...
    private function bookedDatesForMonth(string $monthStart, string $monthEnd, bool $includeRequester): array
    {
        ...
        foreach ($tickets as $ticket) {
            ...
            while ($cursor->lte($end)) {
                ...
                if ($iso >= $monthStart && $iso <= $monthEnd) {
                    $dates[$iso] = $includeRequester ? $name : '';
                }
                $cursor->addDay();
            }
        }
